...
artist = st.selectbox("Select Artist", df["artist(s)_name"].unique())
...
if st.button("🎶 Predict Streams"):
    ...
    # Fill missing numerical features with default values
    for col in X_train.columns:
        if col not in ['danceability_%', 'valence_%', 'energy_%', 'acousticness_%', 'artist(s)_name']:
            new_data.loc[0, col] = 0  # Default values for missing numerical features
